...
def test_get_history_detail_without_pois_returns_lightweight_payload(monkeypatch):
    ...
    monkeypatch.setattr(app_module, "wgs84_to_gcj02", lambda x, y: (x + 0.5, y + 0.25))

    response = asyncio.run(app_module.get_history_detail(7, include_pois=False))
...
def test_get_history_detail_with_pois_keeps_existing_contract(monkeypatch):
    ...
    monkeypatch.setattr(app_module, "wgs84_to_gcj02", lambda x, y: (x + 1.0, y + 2.0))

    response = asyncio.run(app_module.get_history_detail(8, include_pois=True))
...
def test_get_history_pois_returns_poi_only_payload(monkeypatch):
    ...
    monkeypatch.setattr(app_module, "wgs84_to_gcj02", lambda x, y: (x + 0.1, y + 0.2))

    response = asyncio.run(app_module.get_history_pois(11))
...
def test_get_history_detail_and_pois_raise_404_when_record_missing(monkeypatch):
    monkeypatch.setattr(app_module.history_repo, "get_detail", lambda history_id, include_pois=True: None)
    monkeypatch.setattr(app_module.history_repo, "get_pois", lambda history_id: None)

    with pytest.raises(HTTPException) as detail_exc:
        asyncio.run(app_module.get_history_detail(999, include_pois=False))
    with pytest.raises(HTTPException) as pois_exc:
        asyncio.run(app_module.get_history_pois(999))
